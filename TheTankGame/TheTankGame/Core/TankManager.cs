namespace TheTankGame.Core
{
    ...
    public class TankManager : IManager
    {
        ...
        public string Terminate(IList<string> arguments)
        {
            ...
            var currentlyUsedParts = this.parts.Count;//this.vehicles.Values.ToList().Count - this.defeatedVehicles.Count;//this.vehicles.Values.Select(x => x.Model).Count()
            finalResult
                .Append("Currently Used Parts: ");

            //if (vehicles.Count - defeatedVehicles.Count == 0 || defeatedVehicles.Count == 0)
            //{
            //    finalResult.Append("None");
            //}
            //else
            {
                finalResult.Append(currentlyUsedParts);
            }
